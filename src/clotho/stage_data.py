...
def read_stage_params(path: str | Path) -> list[StageInfo]:
    ...
    for _, row in df.iterrows():
        ...
        infos.append(
            StageInfo(
                well=_text(raw, "well", default=""),
                stage=_required_int(raw, "stage"),
                data_file=_text(raw, "data_file", default=""),
                shut_in_time=_text(raw, "shut_in_time", default=""),
...
def read_stage_curve(path: str | Path) -> pd.DataFrame:
    ...
    for name in ["wellhead_pressure_mpa", "rate", "stage_volume", "total_volume"]:
        curve[name] = pd.to_numeric(_column(raw, name), errors="coerce")
...
# 输入：一行原始字段和规范字段名。输出：字符串；找不到时返回默认值。
def _text(raw: dict[str, str], name: str, *, default: str) -> str:
